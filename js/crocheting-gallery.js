...
async function initPhotoGallery() {
    try {
        if (typeof CROCHETING_PHOTOS_DATA !== 'undefined') {
            photoData = CROCHETING_PHOTOS_DATA.photos;
        } else {
            try {
                const response = await fetch('data/crocheting-photos.json');
                if (response.ok) {
...
function createPhotoCard(photo, index) {
    ...
    const imagePath = `images/hobbies/crocheting/${photo.file}`;
    
    photoCard.innerHTML = `
        <img src="${imagePath}" alt="钩织作品 ${photo.id}" loading="lazy">
...
function showEmptyState() {
    ...
    galleryMasonry.innerHTML = `
        <div class="empty-state">
            ...
            <h3>暂无钩织作品</h3>
            <p>请将钩织作品照片放入 <code>images/hobbies/crocheting/</code> 文件夹</p>
...
function saveCaptionsToStorage() {
    ...
    localStorage.setItem('crochetingPhotoCaptions', JSON.stringify(captionsData));
}

function loadSavedCaptions() {
    const savedCaptions = localStorage.getItem('crochetingPhotoCaptions');
