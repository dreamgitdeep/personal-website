...
async function initPhotoGallery() {
    try {
        if (typeof PAINTING_PHOTOS_DATA !== 'undefined') {
            photoData = PAINTING_PHOTOS_DATA.photos;
        } else {
            try {
                const response = await fetch('data/painting-photos.json');
                if (response.ok) {
...
function createPhotoCard(photo, index) {
    ...
    const imagePath = `images/hobbies/painting/${photo.file}`;
    
    photoCard.innerHTML = `
        <img src="${imagePath}" alt="绘画作品 ${photo.id}" loading="lazy">
...
function showEmptyState() {
    ...
    galleryMasonry.innerHTML = `
        <div class="empty-state">
            ...
            <h3>暂无绘画作品</h3>
            <p>请将绘画作品照片放入 <code>images/hobbies/painting/</code> 文件夹</p>
...
function saveCaptionsToStorage() {
    ...
    localStorage.setItem('paintingPhotoCaptions', JSON.stringify(captionsData));
}

function loadSavedCaptions() {
    const savedCaptions = localStorage.getItem('paintingPhotoCaptions');
